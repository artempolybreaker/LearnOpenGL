...
class Model {
    ...
    void ProcessNode(aiNode* node, const aiScene* scene) {
        ...
        for (unsigned int i = 0; i < node->mNumMeshes; i++) {
            ...
            std::cout << "INFO::ASSIMP:: ProcessNode, mesh: " << i << std::endl;
    ...
    unsigned int TextureFromFile(const char *path, const std::string &directory) {
        ...
        // unsigned char* data = stbi_load("/Users/archie-klang/tmp/resources/seed_logo.jpeg", &width, &height, &nrComponents, 0);
        if (data)
        {
            GLenum format;
            if (nrComponents == 1)
