...
int main()
{
    ...
    unsigned char* imageData = stbi_load("./resource/container-tex-0.jpg", &width, &height, &channels, 0);
    ...
    imageData = stbi_load("./resource/container-tex-1.jpg", &width, &height, &channels, 0);
    if (imageData) {
        // generate texture
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, imageData);
        glGenerateMipmap(GL_TEXTURE_2D);
